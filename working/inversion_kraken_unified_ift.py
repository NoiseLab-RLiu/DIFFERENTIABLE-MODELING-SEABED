# -*- coding: utf-8 -*-
"""
Created on Sat Jan 31 23:25:59 2026
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 20 18:02:37 2026
...
# class DebugIFTSolver(torch.autograd.Function):
#     @staticmethod
...
#         # --- DEBUG DIAGNOSTICS ---
#         # Separating Trapped vs Leaky to see who is the culprit
#         with torch.no_grad():
#             k_abs = k.abs()
#             is_leaky = k.imag > 1e-4
#             is_trapped = ~is_leaky
            
#             denom = dF_dk.abs() + 1e-20
            
#             print("\n--- IFT GRADIENT DIAGNOSTICS ---")
#             if is_trapped.any():
#                 print(f"TRAPPED MODES ({is_trapped.sum().item()}):")
#                 print(f"  |dF/dk| (Stability): Min={denom[is_trapped].min():.2e}, Max={denom[is_trapped].max():.2e}")
            
#             if is_leaky.any():
#                 print(f"LEAKY MODES   ({is_leaky.sum().item()}):")
#                 print(f"  |dF/dk| (Stability): Min={denom[is_leaky].min():.2e}, Max={denom[is_leaky].max():.2e}")
                
#                 # Check for Explosion Risk
#                 ratio = dF_dcb.abs() / denom
#                 print(f"  Gradient Ratio |dF/dp|/|dF/dk|: Max={ratio[is_leaky].max():.2e}") # Changed to .2e
...
class DebugIFTSolver(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        with torch.enable_grad():
            ...
            # Reconstruct Characteristic Equation F
            k0 = omega / c_w
            kb_complex = (omega / cb_vec) + 1j * ((ab_vec * (FREQ_GLOBAL/cb_vec))/8.686)
            kz_w = torch.sqrt(k0**2 - k**2)
            kz_b = torch.sqrt(kb_complex**2 - k**2)
            ...
            # Compute Partials (Retain graph for multiple calls)
            dF_dk = torch.autograd.grad(F, k, torch.ones_like(F), create_graph=False, retain_graph=True)[0]
            dF_dcb = torch.autograd.grad(F, cb_vec, torch.ones_like(F), create_graph=False, retain_graph=True)[0]
            dF_drb = torch.autograd.grad(F, rb_vec, torch.ones_like(F), create_graph=False, retain_graph=True)[0]
            dF_dab = torch.autograd.grad(F, ab_vec, torch.ones_like(F), create_graph=False, retain_graph=False)[0]

        # 2. APPLY FIX: Clamp Denominator Magnitude
        denom = dF_dk
        denom_mag = denom.abs()
        
        # Threshold: 1e4 is usually a good balance. 
        # Slopes steeper than this are treated as 1e4 to keep gradients flowing.
        CLAMP_THRESHOLD = 1e4 
        
        is_stiff = denom_mag > CLAMP_THRESHOLD
        
        # Create scaling factor (1.0 for normal modes, <1.0 for stiff modes)
        scale_factor = torch.ones_like(denom_mag)
        scale_factor[is_stiff] = CLAMP_THRESHOLD / denom_mag[is_stiff]
        
        # Apply scaling (preserves complex phase, only reduces magnitude)
        denom_clamped = denom * scale_factor

        # 3. DEBUG DIAGNOSTICS (Updated to show effect of clamping)
        with torch.no_grad():
            k_abs = k.abs()
            is_leaky = k.imag > 1e-4
            is_trapped = ~is_leaky
            
            # Calculate Ratio using CLAMPED denominator
            ratio = dF_dcb.abs() / (denom_clamped.abs() + 1e-20)
            
            # print("\n--- IFT GRADIENT DIAGNOSTICS (CLAMPED) ---")
            # if is_trapped.any():
            #     print(f"TRAPPED MODES ({is_trapped.sum().item()}):")
            #     print(f"  Original |dF/dk|: Max={denom_mag[is_trapped].max():.2e}")
            #     print(f"  Resulting Gradient Ratio: Max={ratio[is_trapped].max():.2e}")
            
            # if is_leaky.any():
            #     print(f"LEAKY MODES   ({is_leaky.sum().item()}):")
            #     print(f"  Original |dF/dk|: Max={denom_mag[is_leaky].max():.2e}")
            #     print(f"  **Resulting Gradient Ratio**: Max={ratio[is_leaky].max():.2e}")
                
            # print("------------------------------------------\n")

        # 4. Apply IFT Formula with Clamped Denominator
        # dk/dp = - (dF/dp) / (dF/dk)_clamped
...
# 1. THE ROBUST IFT SOLVER (Complex + Clamping)
# This solver respects the Imaginary part of Trapped modes!
class IFTSolver(torch.autograd.Function):
    @staticmethod
    def forward(ctx, cb, rho_b, alpha_b, coarse_mode):
        # Run the standard complex solver (keeps imaginary parts!)
        with torch.no_grad():
            k_star = unified_complex_solver(cb, rho_b, alpha_b, coarse_mode=coarse_mode)
    ...
    def backward(ctx, grad_output):
        ...
        # CLAMPING FIX (Essential for Leaky Modes)
        denom = dF_dk
        denom_mag = denom.abs()
        CLAMP_THRESHOLD = 1e4 
        is_stiff = denom_mag > CLAMP_THRESHOLD
...
def forward_model(c_b, rho_b, alpha_b, r_range, stage=1):
    # 1. SELECT SOLVER
    if stage == 1:
        # Stage 1: Coarse Mode (Real Scan) - Autograd
        # Finding the basin doesn't require high precision IFT
        k_complex = unified_complex_solver(c_b, rho_b, alpha_b, coarse_mode=True)
    else:
        # Stage 2: Complex Mode - USE CLAMPED IFT
        # We pass False for coarse_mode to get Complex Roots
    ...
    # 2. PHYSICS FILTERS
    if stage == 1:
        # Stage 1: Keep Ghost, Remove Leaky
        k_complex = k_complex[k_complex.imag < 1e-5]
    else:
        # Stage 2: Keep Trapped + Leaky, Remove Ghost
        # This matches your Autograd script logic exactly
...
#         k_complex = unified_complex_solver(c_b, rho_b, alpha_b, coarse_mode=True)
#     elif stage ==2 :
#         k_complex = DebugIFTSolver.apply(c_b, rho_b, alpha_b, False) # False = complex/fine mode
#         #k_complex = IFTSolver.apply(c_b, rho_b, alpha_b, False)
#     else:
#         # Stage 3: Use Complex Grid (Trapped + Leaky)
#         k_complex = unified_complex_solver(c_b, rho_b, alpha_b, coarse_mode=False)
...
def forward_model(c_b, rho_b, alpha_b, r_range, stage=1):
    # 1. SELECT SOLVER
    if stage == 1:
        # Stage 1: Coarse Mode (Real Scan) - Standard Autograd
        # Returns raw output (Ghost + Trapped)
        k_complex = unified_complex_solver(c_b, rho_b, alpha_b, coarse_mode=True)
    else:
        # Stage 2: Complex Mode - USE CLAMPED IFT
        # Returns raw output (Trapped + Leaky)
        k_complex = IFTSolver.apply(c_b, rho_b, alpha_b, False)
    ...
    # 2. PHYSICS FILTERS
    if stage == 1:
        # STAGE 1: NO FILTERS (As requested)
        # We use the raw output of the coarse solver
        pass 
        
    else:
        # STAGE 2: Remove Ghost Only
        # We need to remove the singularity artifact near k0
        k0 = OMEGA_GLOBAL / C_W_GLOBAL
        is_ghost = torch.abs(k_complex.real - k0) < 2e-4
        k_complex = k_complex[~is_ghost]
...
print("\n>>> STAGE 2: Bridge (Clean Trapped)")
optimizer = torch.optim.Rprop([cb_opt, rb_opt, ab_opt], lr=0.02)#0.02 WORKS!
